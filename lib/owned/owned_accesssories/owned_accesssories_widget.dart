...
import 'owned_accesssories_model.dart';
export 'owned_accesssories_model.dart';
...
class OwnedAccesssoriesWidget extends StatefulWidget {
  ...
  State<OwnedAccesssoriesWidget> createState() =>
      _OwnedAccesssoriesWidgetState();
}

class _OwnedAccesssoriesWidgetState extends State<OwnedAccesssoriesWidget> {
  late OwnedAccesssoriesModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => OwnedAccesssoriesModel());

    logFirebaseEvent('screen_view',
        parameters: {'screen_name': 'OwnedAccesssories'});
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: InkWell(
            ...
            onTap: () async {
              logFirebaseEvent('OWNED_ACCESSSORIES_Icon_ugv4ef7l_ON_TAP');
                ...
                child: Align(
                  ...
                  child: AuthUserStreamWidget(
                    builder: (context) => Builder(
                      builder: (context) {
                        final accessoriesOwn =
                            (currentUserDocument?.ownedAccessories.toList() ??
                                    [])
                                .toList();
                        ...
                        return ListView.builder(
                          ...
                          itemCount: accessoriesOwn.length,
                          itemBuilder: (context, accessoriesOwnIndex) {
                            ...
                            return Padding(
                              ...
                              child: InkWell(
                                ...
                                onTap: () async {
                                  logFirebaseEvent(
                                      'OWNED_ACCESSSORIES_PAGE_menuItem_ON_TAP');
                                  logFirebaseEvent('menuItem_backend_call');

                                  await currentUserReference!
                                      .update(createUsersRecordData(
                                    selectedAccessories: accessoriesOwnItem,
                                  ));
                                      ...
                                      children: [
                                        FlutterFlowWebView(
                                          content: valueOrDefault<String>(
                                            'https://avataaars.io/?avatarStyle=Circle&topType=${valueOrDefault(currentUserDocument?.selectedTop, '')}&accessoriesType=$accessoriesOwnItem&hairColor=${valueOrDefault(currentUserDocument?.selectedHairColors, '')}&facialHairType=${valueOrDefault(currentUserDocument?.selectedFacialHairs, '')}&clotheType=${valueOrDefault(currentUserDocument?.selectedClothes, '')}&eyeType=${valueOrDefault(currentUserDocument?.selectedEyes, '')}&eyebrowType=${valueOrDefault(currentUserDocument?.selectedEyebrows, '')}&mouthType=${valueOrDefault(currentUserDocument?.selectedMouth, '')}&skinColor=${valueOrDefault(currentUserDocument?.selectedSkin, '')}&hatColor=${valueOrDefault(currentUserDocument?.selectedHatColors, '')}&facialHairColor=${valueOrDefault(currentUserDocument?.selectedFacialHairColors, '')}&clotheColor=${valueOrDefault(currentUserDocument?.selectedColorFabric, '')}',
